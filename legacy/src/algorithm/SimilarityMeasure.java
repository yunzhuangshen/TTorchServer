 ...
 * return 1;
 * };
 * SimilarityMeasure<TorVertex> SIM_MEASURE = new SimilarityMeasure<>(distFunc, comparator);
 *
 * @author forrest0402
 * @Description
 * @date 12/11/2017
 */
public class SimilarityMeasure<T> {

    private static Logger logger = LoggerFactory.getLogger(SimilarityMeasure.class);

    final DistanceFunction distFunc;

    final Comparator<T> comparator;

    public SimilarityMeasure(DistanceFunction<T, T> distFunc, Comparator<T> comparator) {
    ...
    public double fastLongestCommonSubsequence(List<T> T1, List<T> T2, int theta, double bestSoFar) {
        ...
        for (int i = 1; i < T1.size(); ++i) {
            for (int j = 1; j < T2.size(); ++j) {
                if (Math.abs(i - j) <= theta) {
                    ...
                    if (dpInts[i][j] + Math.min(T1.size() - i - 1, T2.size() - j - 1) < bestSoFar)
                        return dpInts[i][j] + Math.min(T1.size() - i - 1, T2.size() - j - 1);
                }
            }
        }
    ...
    public double DynamicTimeWarping(List<T> T1, List<T> T2) {
        ...
        for (int i = 1; i <= T1.size(); ++i) {
            for (int j = 1; j <= T2.size(); ++j) {
                dpInts[i][j] = distFunc.apply(T1.get(i - 1), T2.get(j - 1)) + min3(dpInts[i - 1][j - 1], dpInts[i - 1][j], dpInts[i][j - 1]);
            }
        }
