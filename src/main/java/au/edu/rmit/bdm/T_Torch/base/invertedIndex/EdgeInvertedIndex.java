package au.edu.rmit.bdm.T_Torch.base.invertedIndex;

import au.edu.rmit.bdm.T_Torch.base.PathQueryIndex;
import au.edu.rmit.bdm.T_Torch.base.TopKQueryIndex;
import au.edu.rmit.bdm.T_Torch.base.model.TorEdge;
 ...
 * value: pairs( bdm id -- position of that key in the bdm)
 *
 */
public class EdgeInvertedIndex extends InvertedIndex implements PathQueryIndex, TopKQueryIndex {
    ...
    public List<String> findByStrictPath(List<LightEdge> edges) {
        ...
        //key is bdm id, value is number of different query edges
     ...
     * Find top K trajectories that has the max score( similarity) against query bdm( represented by a list of edges).
     * Used in efficiency test.
     *
     * @param edgeQuery a list of edges representing a query
     * @param k number of results returned
     *
     * @return A list of results of type Integer meaning ids of bdm.
     */
    @Override
    public <T extends TrajEntry> List<String> findTopK(int k, List<T> pointQuery, List<LightEdge> edgeQuery){

        // 1. compute upper bound for each candidate bdm

        // key for bdm id, value for its upper bound
        Map<String, Double> candidateUpperBound = new HashMap<>();
        // key for bdm id, value for the list of edges overlapped with the query edge.
        ...
        for (LightEdge queryEdge : edgeQuery) {
            ...
            //key for bdm hash, value for position
            for (Pair pair : trajPosMap) {

                String trajId = String.valueOf(pair.trajid);
                //calculate upper bound for each bdm
                candidateUpperBound.merge(trajId, queryEdge.length, (a, b) -> b + a);
                //re-construct every bdm, need to reorder in the next steps
        ...
        // 2. prune candidates.
        // compute exact score from highest upper bound bdm to lowest,
        // If the current k ranked bdm exact bound is higher than current candidate bdm upper bound,
        // terminate.
        ...
        // key for bdm id, value for its exact score
     ...
     *
     * @param qEdges          edges representing query bdm
     * @param cEdges          edges representing sub candidate bdm
     * @param theta           For instance, theta is 5. If the 3rd edge in query bdm matches the 11th edge in candidate bdm,
