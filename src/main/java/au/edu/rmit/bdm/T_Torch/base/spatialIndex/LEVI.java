package au.edu.rmit.bdm.T_Torch.base.spatialIndex;
...
import au.edu.rmit.bdm.T_Torch.base.helper.GeoUtil;
...
import au.edu.rmit.bdm.T_Torch.queryEngine.similarity.SimilarityFunction;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.*;

import static au.edu.rmit.bdm.T_Torch.queryEngine.similarity.SimilarityFunction.*;
...
public class LEVI implements WindowQueryIndex, TopKQueryIndex {
    ...
    private <T extends TrajEntry> List<String> topKwithDTW(int k, List<T> pointQuery) {
        ...
        while (check == 0) {
            ...
            //each query point match with the nearest point of a bdm,
            // and the lower bound is the maximum distance between a query and existing points of a bdm
            ...
            for (int i = 0; i < querySize; i++) {
                ...
                //findMoreVertices the nearest pair between a bdm and query queryVertex
                //bdm hash, queryVertex hash vertices
            ...
            while (!rankedCandidates.isEmpty()) {
                ...
                if (topKHeap.size() < k) {
                    ...
                    if (++j % 1500 == 0 || bestKthSoFar > curUpperBound)
                        logger.info("have processed {} trajectories, current {}th bdm upper bound: {}, " +
                                        "top kth bdm real score: {}, current unseen bdm upper bound: {}",
