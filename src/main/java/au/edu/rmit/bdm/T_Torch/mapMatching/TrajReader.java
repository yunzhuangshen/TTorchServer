...
import au.edu.rmit.bdm.T_Torch.base.model.TrajNode;
...
public class TrajReader {
     ...
     *                 This file could be null and if this is the case, the program will leave time field in bdm model blank.
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    logger.warn("bdm id {} is excluded, either too short or contain illegal chars", trajId);
                    continue;
                }
                ...
                for (int i = 0; i < trajTuples.length; i++){
                    ...
                    try {
                        ...
                        logger.warn("node of bdm ID {} contains illegal gps location, that node will be excluded. " + trajId);
                        continue;
                    }
                    ...
                    if (hasDate) {
                        ...
                        try {
                            ...
                            logger.warn("node of bdm ID {} contains illegal timestamp, that node will be excluded. " + trajId);
                            continue;
                        }
