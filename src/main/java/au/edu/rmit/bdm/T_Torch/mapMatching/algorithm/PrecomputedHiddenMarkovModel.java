...
import au.edu.rmit.bdm.T_Torch.mapMatching.model.TorVertex;
...
public class PrecomputedHiddenMarkovModel implements Mapper {
    ...
    public Trajectory<TowerVertex> match(Trajectory<? extends TrajEntry> rawTraj) throws Exception {
        ...
        // at least, a bdm should be defined with 2 points.
        if (rawTraj.size() < 2) {
            logger.debug("remote unqualified bdm id: {}", rawTraj.id);
            throw new IllegalStateException();
        }

        logger.debug("start matching for bdm id: {}", rawTraj.id);

        // find candidates for each entry of the raw bdm, compute emission probability
        ...
        for (int i = 1; i < mapMatchedVertices.size()-1; ++i) {
            ...
            {
                logger.debug("bdm id: {}, cannot find shortest edges", rawTraj.id);
                throw new IllegalStateException();
            }
...
//        }
    ...
    /**
     * only include bdm node that is either the first one,
     * the last one, or not within two GPS standard deviation with each other.
     * 
     * @param in the bdm to be checked
     * @return filtered bdm
     */
    ...
    /**
     * The class contains candidate vertex for the given bdm node,
