package au.edu.rmit.bdm.T_Torch.queryEngine;

import au.edu.rmit.bdm.T_Torch.base.Torch;
import au.edu.rmit.bdm.T_Torch.base.model.Coordinate;
...
import au.edu.rmit.bdm.T_Torch.queryEngine.query.QueryProperties;
import au.edu.rmit.bdm.T_Torch.queryEngine.query.Query;
import au.edu.rmit.bdm.T_Torch.queryEngine.query.QueryPool;
...
/**
 * Engine class contains high level APIs to query on bdm data-set
 */
public class Engine {
     ...
     *
     * The subroutine will first map convert bdm to map-matched bdm,
     ...
     *
     *  Given a raw bdm as query, the subroutine will first map convert bdm to map-matched bdm,
     *  and then find all the trajectories in data-set that at least has a same edge( road segment) with the query bdm.
     ...
     *
     *  Given a raw bdm as query, the subroutine will first map convert bdm to map-matched bdm,
     *  and then find all the trajectories in data-set that contain all same edges( road segment) with the query bdm.
     *  Same as previous, If the raw query can not be map-matched, {@code QueryResult} will indicates error with be returned.
     *
     * @param raw A list of points representing the query.
     *            T-Torch provides your simple class {@code Coordinate}
    ...
    public static class Builder{
        ...
        /**
         * todo support top K over raw bdm data-set.
